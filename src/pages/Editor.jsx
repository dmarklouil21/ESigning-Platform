...
// --- Memoized PDF Viewer ---
const MemoizedPDF = memo(({ fileUrl, pageNumber, scale, onLoadSuccess }) => {
  return (
    <Document 
      file={fileUrl} 
...
const Editor = () => {
  ...
  const updateElement = useCallback((id, props) => {
    setSignatures(prev => prev.map(s => s.id === id ? { ...s, ...props } : s));
  }, []);
  ...
  const addTextElement = (text) => {
    ...
    const newElement = { id: Date.now(), type: 'text', text: safeText, x: 50, y: 100, width: Math.max(100, safeText.length * 10), height: 30, page: pageNumber };
  ...
  const handleSaveSignature = (url) => {
    ...
    img.onload = () => {
      ...
      const newSig = { id: Date.now(), type: 'image', url, x: 50, y: 100, width: baseWidth, height: calculatedHeight, page: pageNumber };
          ...
          <div className="min-w-fit min-h-full flex flex-col items-center p-4 md:p-8 pb-40">
            <div ref={pdfContainerRef} className="relative inline-block shadow-2xl border border-slate-300 bg-white select-none">
              {documentData?.fileUrl && (
                <MemoizedPDF fileUrl={documentData.fileUrl} pageNumber={pageNumber} scale={scale} onLoadSuccess={onDocumentLoadSuccess} />
              )}
